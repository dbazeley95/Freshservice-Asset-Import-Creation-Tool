...
2. Fill in the **Shared Defaults** (Company, Location, Product, Cost,
   Warranty, Asset State, Acquisition Date, and any type-specific fields like
   Processor/Memory/Disk or OS/Storage). These values get copied onto new
   rows as you add them.
3. Paste one serial number per line into **Bulk Add from Serial Numbers**,
...
4. Every generated row is independently editable in the table — tweak any
   cell by hand, or use **Add Blank Row** for one-off manual entries.
   Required fields with no value are outlined in red.
5. Click **Download CSV** to save a file with headers matching the
